...
plot.transScore.mix <- function(transScore, control = 'WT', combine.arms = T, interArm = F){
  require(tidyr)
  require(dplyr)
  require(ggplot2)
  require(ggpubr)
  ...
  for(sample in samples){
    ...
    plt <- ggplot(transScore2 %>% mutate(chrom = factor(chrom, levels = chroms), 
      ...
      scale_colour_gradient2(low='blue', mid='white', high='red', na.value = 'grey', midpoint = 0) +
      geom_vline(xintercept = 0, color='black') +
      labs(x = '', y = 'Chromosome', fill = paste0('log2(', colnames(transScore)[sample], '/', control, ')')) +
      scale_x_continuous(labels = function(x) format(x/1e6)) +
      scale_y_discrete(labels = function(x) format(sub('chr', '', x))) +
      guides(colour=F) +
      GENOVA:::GENOVA_THEME() +
  ...
  plot_logfc <- 
      ggplot(data = transScore2,
                   aes(x = cenBin)) +
        geom_smooth(aes(x=cenBin, y=log_fc), colour=ifelse(median(transScore2$log_fc) >= 0, 'red', 'blue')) +
        #geom_point(aes(x = cenBin, y = trans.ratio, colour = sample), size = 0.3) + #TODO:make it reusable (names)
        #ylim(-0.5, 0.5) +
        #scale_colour_manual(name = 'Cell line', values = c('grey', 'black')) +
        #scale_fill_manual(name = 'Cell line', values = c('grey', 'black')) +
        #geom_vline(xintercept = 0, color='black') +
  ...
  final <- ggarrange(plotlist = c(plots_perchrom, plots_pileup), ncol = length(samples), nrow=2, 
            align = 'hv', heights = c(9, 2))
  annotate_figure(final, top = text_grob(paste('Change in proportion of', mode, 'interactions')))
}
